...
namespace sek
{
	...
	[[nodiscard]] inline basic_vec<T, N, A> blend(const basic_vec<T, N, A> &a, const basic_vec<T, N, A> &b, const basic_vec_mask<T, N, A> &m)
	{
	...
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline basic_vec_mask<T, N, A> blend(const basic_vec_mask<T, N, A> &a, const basic_vec_mask<T, N, A> &b, const basic_vec_mask<T, N, A> &m)
	{
		return {dpm::blend(to_simd(a), to_simd(b), to_simd(m))};
	}
	...
	public:
		...
		where_expression(const M &mask, T &data) noexcept : m_data(to_simd(mask), to_simd(data)) {}

		/** Returns a copy of the underlying vector or mask. */
		[[nodiscard]] inline value_type operator+() const && noexcept { return {+m_data}; }
		[[nodiscard]] inline value_type operator-() const && noexcept { return {-m_data}; }
		[[nodiscard]] inline value_type operator~() const && noexcept requires (requires(T x){ ~x; }) { return {~m_data}; }
		...
		[[nodiscard]] inline value_type value_or(const value_type &x) const && noexcept { return {dpm::blend(to_simd(x), m_data)}; }

		inline void operator++(int) && noexcept requires (requires(simd_expression &&data){ data++; }) { m_data++; }
		inline void operator--(int) && noexcept requires (requires(simd_expression &&data){ data--; }) { m_data--; }
		inline void operator++() && noexcept requires (requires(simd_expression &&data){ ++data; }) { ++m_data; }
		inline void operator--() && noexcept requires (requires(simd_expression &&data){ --data; }) { --m_data; }

		template<typename U>
		inline void operator+=(U &&value) && noexcept requires (requires(simd_expression &&data){ data += value; }) { m_data += std::forward<U>(value); }
		template<typename U>
		inline void operator-=(U &&value) && noexcept requires (requires(simd_expression &&data){ data -= value; }) { m_data -= std::forward<U>(value); }
	...
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline where_expression<basic_vec_mask<T, N, A>, basic_vec<T, N, A>> where(const basic_vec_mask<T, N, A> &m, basic_vec<T, N, A> &v) noexcept { return {m, v}; }
	/** @copydoc where */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline where_expression<basic_vec_mask<T, N, A>, const basic_vec<T, N, A>> where(const basic_vec_mask<T, N, A> &m, const basic_vec<T, N, A> &v) noexcept { return {m, v}; }
	/** Creates a where expression used to select elements of vector mask \a v using vector mask \a m. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline where_expression<basic_vec_mask<T, N, A>, basic_vec_mask<T, N, A>> where(const basic_vec_mask<T, N, A> &m, basic_vec_mask<T, N, A> &v) noexcept { return {m, v}; }
	/** @copydoc where */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline where_expression<basic_vec_mask<T, N, A>, const basic_vec_mask<T, N, A>> where(const basic_vec_mask<T, N, A> &m, const basic_vec_mask<T, N, A> &v) noexcept { return {m, v}; }

	/** Replaces elements of vector \a a with selected elements of where expression \a b. */
	template<typename T, std::size_t N, typename A, typename M>
	[[nodiscard]] inline basic_vec<T, N, A> blend(const basic_vec<T, N, A> &a, const where_expression<M, basic_vec<T, N, A>> &b) { return std::move(b).value_or(a); }
	/** @copydoc blend */
	template<typename T, std::size_t N, typename A, typename M>
	[[nodiscard]] inline basic_vec<T, N, A> blend(const basic_vec<T, N, A> &a, const where_expression<M, const basic_vec<T, N, A>> &b) { return std::move(b).value_or(a); }
	/** Replaces elements of vector mask \a a with selected elements of where expression \a b. */
	template<typename T, std::size_t N, typename A, typename M>
	[[nodiscard]] inline basic_vec_mask<T, N, A> blend(const basic_vec_mask<T, N, A> &a, const where_expression<M, basic_vec_mask<T, N, A>> &b) { return std::move(b).value_or(a); }
	/** @copydoc blend */
	template<typename T, std::size_t N, typename A, typename M>
	[[nodiscard]] inline basic_vec_mask<T, N, A> blend(const basic_vec_mask<T, N, A> &a, const where_expression<M, const basic_vec_mask<T, N, A>> &b) { return std::move(b).value_or(a); }

	/** Calculates a reduction of selected elements from \a x using \a binary_op and identity element \a identity. */
	template<typename M, typename V, typename Op>
	[[nodiscard]] inline typename V::value_type reduce(const where_expression<M, V> &x, typename V::value_type identity, Op binary_op) { return dpm::reduce(x.m_data, identity, std::move(binary_op)); }
	...
	[[nodiscard]] inline typename V::value_type reduce(const where_expression<M, V> &x, Op binary_op) noexcept { return reduce(x, typename V::value_type{0}, std::move(binary_op)); }
	...
	[[nodiscard]] inline typename V::value_type reduce(const where_expression<M, V> &x, Op binary_op) noexcept { return reduce(x, typename V::value_type{1}, std::move(binary_op)); }
	...
	[[nodiscard]] inline typename V::value_type reduce(const where_expression<M, V> &x, Op binary_op) noexcept
	{
	...
	template<typename M, typename V>
	[[nodiscard]] inline typename V::value_type hmin(const where_expression<M, V> &x) noexcept { return dpm::hmin(x.m_data); }
	/** Finds the horizontal maximum of all selected elements in \a x. */
	template<typename M, typename V>
	[[nodiscard]] inline typename V::value_type hmax(const where_expression<M, V> &x) noexcept { return dpm::hmax(x.m_data); }

	/** Finds the horizontal sum of all selected elements in \a x. */
	template<typename M, typename V>
	[[nodiscard]] inline typename V::value_type hadd(const where_expression<M, V> &x) noexcept { return dpm::hadd(x.m_data); }
	/** Finds the horizontal product of all selected elements in \a x. */
	template<typename M, typename V>
	[[nodiscard]] inline typename V::value_type hmul(const where_expression<M, V> &x) noexcept { return dpm::hmul(x.m_data); }

	/** Finds the horizontal bitwise AND of all selected elements in \a x. */
	template<typename M, typename V>
	[[nodiscard]] inline typename V::value_type hand(const where_expression<M, V> &x) noexcept { return dpm::hand(x.m_data); }
	/** Finds the horizontal bitwise XOR of all selected elements in \a x. */
	template<typename M, typename V>
	[[nodiscard]] inline typename V::value_type hxor(const where_expression<M, V> &x) noexcept { return dpm::hxor(x.m_data); }
	/** Finds the horizontal bitwise OR of all selected elements in \a x. */
	template<typename M, typename V>
	[[nodiscard]] inline typename V::value_type hor(const where_expression<M, V> &x) noexcept { return dpm::hor(x.m_data); }
}