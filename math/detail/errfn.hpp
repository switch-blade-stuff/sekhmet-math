...
namespace sek
{
	...
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> erf(const basic_vec<T, N, A> &x) noexcept { return {dpm::erf(to_simd(x))}; }
	/** @copydoc erfc
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> erfc(const basic_vec<T, N, A> &x) noexcept { return {dpm::erfc(to_simd(x))}; }
	/** @copydoc tgamma
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> tgamma(const basic_vec<T, N, A> &x) noexcept { return {dpm::tgamma(to_simd(x))}; }
	/** @copydoc lgamma
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> lgamma(const basic_vec<T, N, A> &x) noexcept { return {dpm::lgamma(to_simd(x))}; }
}