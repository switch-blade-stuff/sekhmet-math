...
namespace sek
{
	...
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> exp(const basic_vec<T, N, A> &x) noexcept { return {dpm::exp(to_simd(x))}; }
	/** @copydoc exp2
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> exp2(const basic_vec<T, N, A> &x) noexcept { return {dpm::exp2(to_simd(x))}; }
	/** @copydoc expm1
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> expm1(const basic_vec<T, N, A> &x) noexcept { return {dpm::expm1(to_simd(x))}; }
	/** @copydoc log
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> log(const basic_vec<T, N, A> &x) noexcept { return {dpm::log(to_simd(x))}; }
	/** @copydoc log1p
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> log1p(const basic_vec<T, N, A> &x) noexcept { return {dpm::log1p(to_simd(x))}; }
	/** @copydoc log10
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> log10(const basic_vec<T, N, A> &x) noexcept { return {dpm::log10(to_simd(x))}; }
	/** @copydoc log2
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> log2(const basic_vec<T, N, A> &x) noexcept { return {dpm::log2(to_simd(x))}; }
}