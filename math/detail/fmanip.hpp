...
namespace sek
{
	...
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> frexp(const basic_vec<T, N, A> &x, basic_vec<int, N, A> &exp) noexcept { return {dpm::frexp(to_simd(x), to_simd(exp))}; }
	 ...
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> ldexp(const basic_vec<T, N, A> &x, const basic_vec<int, N, A> &exp) noexcept { return {dpm::ldexp(to_simd(x), to_simd(exp))}; }
	/** @copydoc scalbn
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> scalbn(const basic_vec<T, N, A> &x, const basic_vec<int, N, A> &exp) noexcept { return {dpm::scalbn(to_simd(x), to_simd(exp))}; }
	/** @copydoc scalbln
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> scalbln(const basic_vec<T, N, A> &x, const basic_vec<int, N, A> &exp) noexcept { return {dpm::scalbln(to_simd(x), to_simd(exp))}; }
	 ...
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> logb(const basic_vec<T, N, A> &x) noexcept { return {dpm::logb(to_simd(x))}; }
	...
	[[nodiscard]] inline vec<detail::promote_t<T0, T1>, N, A> nextafter(const basic_vec<T0, N, A> &from, const basic_vec<T1, N, A> &to) noexcept { return {dpm::nextafter(to_simd(from), to_simd(to))}; }
	/** @copydoc nextafter
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T0, typename T1, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T0, T1>, N, A> nexttoward(const basic_vec<T0, N, A> &from, const basic_vec<long double, N, A> &to) noexcept { return {dpm::nexttoward(to_simd(from), to_simd(to))}; }
	 ...
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T0, typename T1, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T0, T1>, N, A> copysign(const basic_vec<T0, N, A> &x, const basic_vec<T1, N, A> &sign) noexcept { return {dpm::copysign(to_simd(x), to_simd(sign))}; }
	 ...
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> ldexp(const basic_vec<T, N, A> &x, int exp) noexcept { return {dpm::ldexp(to_simd(x), exp)}; }
	/** @copydoc scalbn
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> scalbn(const basic_vec<T, N, A> &x, int exp) noexcept { return {dpm::scalbn(to_simd(x), exp)}; }
	/** @copydoc scalbln
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> scalbln(const basic_vec<T, N, A> &x, long exp) noexcept { return {dpm::scalbln(to_simd(x), exp)}; }
	/** @copydoc copysign
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T0, typename T1, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T0, T1>, N, A> copysign(const basic_vec<T0, N, A> &x, T1 sign) noexcept { return {dpm::copysign(to_simd(x), sign)}; }
}