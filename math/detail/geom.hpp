...
namespace sek
{
	...
	namespace detail
	{
		...
		template<typename T>
		[[nodiscard]] DPM_FORCEINLINE T do_sqrt(T dp) noexcept { return static_cast<T>(std::sqrt(dp)); }
		template<typename T>
		[[nodiscard]] DPM_FORCEINLINE T do_rsqrt(T dp) noexcept { return static_cast<T>(1) / do_sqrt(dp); }
#ifdef __SSE__
		[[nodiscard]] DPM_FORCEINLINE float do_sqrt(float dp) noexcept { return _mm_cvtss_f32(_mm_sqrt_ss(_mm_set_ss(dp))); }
		[[nodiscard]] DPM_FORCEINLINE float do_rsqrt(float dp) noexcept { return _mm_cvtss_f32(_mm_rsqrt_ss(_mm_set_ss(dp))); }
#endif
#ifdef __SSE2__
		[[nodiscard]] DPM_FORCEINLINE double do_sqrt(double dp) noexcept
		{
		...
		template<typename T>
		[[nodiscard]] DPM_FORCEINLINE T do_fma(T a, T b, T c) noexcept { return a * b + c; }
#if defined(__FMA__) || (defined(_MSC_VER) && defined(__AVX2__))
		[[nodiscard]] DPM_FORCEINLINE float do_fma(float a, float b, float c) noexcept
		{
		...
		[[nodiscard]] DPM_FORCEINLINE double do_fma(double a, double b, double c) noexcept
		{
	...
	[[nodiscard]] DPM_FORCEINLINE T magn(const basic_vec<T, N, A> &x) noexcept { return detail::do_sqrt(dot(x, x)); }
	/** Calculates the Euclidean distance between vectors \a a and \a b. */
	template<std::floating_point T, std::size_t N, typename A>
	[[nodiscard]] DPM_FORCEINLINE T dist(const basic_vec<T, N, A> &a, const basic_vec<T, N, A> &b) noexcept { return magn(a - b); }
	/** @copydoc magn
	...
	[[nodiscard]] DPM_FORCEINLINE typename Promoted::value_type magn(const basic_vec<T, N, A> &x) noexcept { return magn(Promoted{x}); }
	/** @copydoc dist
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T0, typename T1, std::size_t N, typename A, typename Promoted = vec<detail::promote_t<T0, T1>, N, A>>
	[[nodiscard]] DPM_FORCEINLINE typename Promoted::value_type dist(const basic_vec<T0, N, A> &a, const basic_vec<T1, N, A> &b) noexcept { return magn(Promoted{a}, Promoted{b}); }
	 ...
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A, typename Promoted = vec<detail::promote_t<T>, N, A>>
	[[nodiscard]] DPM_FORCEINLINE Promoted normalize(const basic_vec<T, N, A> &x) noexcept { return normalize(Promoted{x}); }
	...
	[[nodiscard]] DPM_FORCEINLINE Promoted faceforward(const basic_vec<T0, N, A> &n, const basic_vec<T1, N, A> &i, const basic_vec<T2, N, A> &r) noexcept
	{
	 ...
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T0, typename T1, std::size_t N, typename A, typename Promoted = vec<detail::promote_t<T0, T1>, N, A>>
	[[nodiscard]] DPM_FORCEINLINE Promoted reflect(const basic_vec<T0, N, A> &i, const basic_vec<T1, N, A> &n) noexcept
	{
	 ...
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T0, typename T1, typename T2, std::size_t N, typename A, typename Promoted = vec<detail::promote_t<T0, T1, T2>, N, A>>
	[[nodiscard]] DPM_FORCEINLINE Promoted refract(const basic_vec<T0, N, A> &i, const basic_vec<T1, N, A> &n, T2 e) noexcept
	{
