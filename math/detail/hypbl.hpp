...
namespace sek
{
	...
	[[nodiscard]] inline basic_vec<T, N, A> sinh(const basic_vec<T, N, A> &x) noexcept { return {dpm::sinh(to_simd(x))}; }
	/** Calculates hyperbolic cosine of elements in vector \a x. */
	template<std::floating_point T, std::size_t N, typename A>
	[[nodiscard]] inline basic_vec<T, N, A> cosh(const basic_vec<T, N, A> &x) noexcept { return {dpm::cosh(to_simd(x))}; }
	/** Calculates hyperbolic tangent of elements in vector \a x. */
	template<std::floating_point T, std::size_t N, typename A>
	[[nodiscard]] inline basic_vec<T, N, A> tanh(const basic_vec<T, N, A> &x) noexcept { return {dpm::tanh(to_simd(x))}; }
	/** Calculates hyperbolic arc-sine of elements in vector \a x. */
	template<std::floating_point T, std::size_t N, typename A>
	[[nodiscard]] inline basic_vec<T, N, A> asinh(const basic_vec<T, N, A> &x) noexcept { return {dpm::asinh(to_simd(x))}; }
	/** Calculates hyperbolic arc-cosine of elements in vector \a x. */
	template<std::floating_point T, std::size_t N, typename A>
	[[nodiscard]] inline basic_vec<T, N, A> acosh(const basic_vec<T, N, A> &x) noexcept { return {dpm::acosh(to_simd(x))}; }
	/** Calculates hyperbolic arc-tangent of elements in vector \a x. */
	template<std::floating_point T, std::size_t N, typename A>
	[[nodiscard]] inline basic_vec<T, N, A> atanh(const basic_vec<T, N, A> &x) noexcept { return {dpm::atanh(to_simd(x))}; }
