...
namespace sek
{
	...
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> sqrt(const basic_vec<T, N, A> &x) noexcept { return {dpm::sqrt(to_simd(x))}; }
	/** @copydoc cbrt
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> cbrt(const basic_vec<T, N, A> &x) noexcept { return {dpm::cbrt(to_simd(x))}; }
	/** @copydoc rsqrt
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> rsqrt(const basic_vec<T, N, A> &x) noexcept { return {dpm::rsqrt(to_simd(x))}; }
	...
	[[nodiscard]] inline vec<detail::promote_t<T0, T1>, N, A> hypot(const basic_vec<T0, N, A> &a, const basic_vec<T1, N, A> &b) noexcept { return {dpm::hypot(to_simd(a), to_simd(b))}; }
	 ...
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T0, typename T1, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T0, T1>, N, A> pow(const basic_vec<T0, N, A> &x, const basic_vec<T1, N, A> &p) noexcept { return {dpm::pow(to_simd(x), to_simd(p))}; }
	/** @copydoc pow
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T0, typename T1, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T0, T1>, N, A> pow(const basic_vec<T0, N, A> &x, T1 p) noexcept { return {dpm::pow(to_simd(x), p)}; }
	 ...
	 * @note Arguments and return type are promoted to `double`, or `long double` if one of the arguments is `long double`. */
	template<typename T, std::size_t N, typename A>
	[[nodiscard]] inline vec<detail::promote_t<T>, N, A> rcp(const basic_vec<T, N, A> &x) noexcept { return {dpm::rcp(to_simd(x))}; }
}