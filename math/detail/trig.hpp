...
namespace sek
{
	...
	[[nodiscard]] inline basic_vec<T, N, A> sin(const basic_vec<T, N, A> &x) noexcept { return {dpm::sin(to_simd(x))}; }
	/** Calculates cosine of elements in vector \a x. */
	template<std::floating_point T, std::size_t N, typename A>
	[[nodiscard]] inline basic_vec<T, N, A> cos(const basic_vec<T, N, A> &x) noexcept { return {dpm::cos(to_simd(x))}; }
	/** Calculates tangent of elements in vector \a x. */
	template<std::floating_point T, std::size_t N, typename A>
	[[nodiscard]] inline basic_vec<T, N, A> tan(const basic_vec<T, N, A> &x) noexcept { return {dpm::tan(to_simd(x))}; }
	/** Calculates arc-sine of elements in vector \a x. */
	template<std::floating_point T, std::size_t N, typename A>
	[[nodiscard]] inline basic_vec<T, N, A> asin(const basic_vec<T, N, A> &x) noexcept { return {dpm::asin(to_simd(x))}; }
	/** Calculates arc-cosine of elements in vector \a x. */
	template<std::floating_point T, std::size_t N, typename A>
	[[nodiscard]] inline basic_vec<T, N, A> acos(const basic_vec<T, N, A> &x) noexcept { return {dpm::acos(to_simd(x))}; }
	/** Calculates arc-tangent of elements in vector \a x. */
	template<std::floating_point T, std::size_t N, typename A>
	[[nodiscard]] inline basic_vec<T, N, A> atan(const basic_vec<T, N, A> &x) noexcept { return {dpm::atan(to_simd(x))}; }
