/*
 * Created by switch_blade on 2023-03-05.
 */
...
namespace sek
{
	...
	/** @brief `xoroshiro` random number engine.
	...
	class xoroshiro : detail::impl_xoroshiro<T, Width>
	{
		using base_t = detail::impl_xoroshiro<T, Width>;
	...
	public:
		...
		constexpr void jump() noexcept { base_t::do_jump(base_t::jmp_short); }
		/** Advances the generator by 2^192. */
		constexpr void long_jump() noexcept { base_t::do_jump(base_t::jmp_long); }

		[[nodiscard]] constexpr bool operator==(const xoroshiro &) const noexcept = default;

	private:
		[[nodiscard]] constexpr auto &state() noexcept { return base_t::m_state; }
		[[nodiscard]] constexpr const auto &state() const noexcept { return base_t::m_state; }
	};

	/** 128-bit alias for the xoroshiro random number generator. */
	template<typename T>
	using xoroshiro128 = xoroshiro<T, 128>;
	/** 256-bit alias for the xoroshiro random number generator. */
	template<typename T>
